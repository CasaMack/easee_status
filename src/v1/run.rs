...
#[instrument]
pub fn get_db_info() -> (Arc<String>, Arc<String>, Arc<String>) {
    ...
    let db_measurement =
        env::var("INFLUXDB_DB_MEASUREMENT").expect("INFLUXDB_DB_MEASUREMENT not set");
    tracing::info!("INFLUXDB_DB_MEASUREMENT: {}", db_measurement);

    (
        Arc::new(db_addr),
        Arc::new(db_name),
        Arc::new(db_measurement),
    )
}

pub fn get_logger() -> (FmtSubscriber<DefaultFields, Format, LevelFilter, NonBlocking>, WorkerGuard){
    ...
    match charger_state {
        Ok(state) => {
            ...
            for charger in state {
                ...
                write_to_db(&client, "power", charger.power, db_measurement.clone()).await;
                ...
                write_to_db(&client, "session", charger.session, db_measurement.clone()).await;
            }
        }
        Err(e) => {
...
async fn write_to_db(client: &Client, name: &str, value: f64, measurement: Arc<String>) {
    ...
    let write_result = client
        .query(variable.into_query(measurement.as_str()))
        .await;
    match write_result {
        Ok(_) => {
